...
| 画像 | 挿入○(PNG・JPEG・**SVG は高精細に変換**)。図形・グラフは Python で描いて貼る | 挿入○(PNG・JPEG)。図形・スパークライン・記号も。グラフは釦の裏で matplotlib が描く |
| コメント | ○(段落単位) | ○ |
...
| Python の持ち運び(マクロの代わり) | — | ○(@save でブックに載せ、実行は檻の中) |
...
ソルバー・ピボットはアプリの中に作りません — polars で集計して帳票へ
差し込みます。グラフの釦はありますが、描いているのは裏方の matplotlib です。
...
リボンに並んでいるコマンドのうち、動くのは writer 77/91、calc 84/121。
