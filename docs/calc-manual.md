...
枠の固定もここ。シートの表示(隠しシート)とインターフェイステーマは灰色。
...
リボンの灰色は残り6つ(139/145 実装)。**方針で灰色のままにしているもの**:

| 灰色のもの | なぜ |
|---|---|
| 範囲に変換する / テーブルのサイズ変更 | 表オブジェクトを持たない設計(書式と式だけ残す) |
| シートの方向(右から左)/ direction | 日本の事務の様式では使わない |
| 配色の変更 / インターフェイステーマ | テーマ色の読み(a:theme)が先。writer と同根の残件 |

以下は**動くもの**と、その代わりの Python:
