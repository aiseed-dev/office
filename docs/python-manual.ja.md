...
| 場所 | 束縛 | サンドボックス |
|---|---|---|
| calc: データ > Python(一行 / .py) | `b` = ブック、`s` = いまのシート | あれば使う(ネットワークあり) |
| calc: `@名前`(**plugins の .py** — 手続きはブックに載らない。2026-08-08 確定) | 同上 | **必ず使う**(ネットワーク無し。`net` で許可) |
| calc: `=PY("関数",…)` + `@計算`(関数だけ `@save 関数名` でブックに載る) | 引数が値で渡る(下記) | 必ず使う(ネットワーク無し) |
| calc / writer: マクロ・プラグイン | calc は `b`/`s`、**writer は `d` = python-docx の文書** | 使う |
| writer: ページの Python(HTML) | `form` = 記入欄の名前→値の辞書 | 必ず使う |
...
- 返り値: スカラ → そのセルへ / **1次元リスト → 下へ展開** /
  **2次元リスト → 右下へスピル**。展開先に他人のデータがあれば
  `#SPILL!` で止まる(潰さない)
- 評価は `@計算` のときだけ・サンドボックスの中。関数の定義は「関数」で
  始まる名前で `@save` したスクリプトの def
...
## サンドボックスの中でできること・できないこと

- 実ファイルは**読み取り専用**、ホームは見えない、書けるのは交換用の
  一時領域だけ。ネットワークは**既定で閉じている** — `@名前 net` とその場で
  打ったときだけ開く(許可はどこにも保存されない)
- 時間制限つき(手続き60秒・=PY の関数30秒)。超えたら止めてそう言う
...
【実行環境】bubblewrap のサンドボックスの中。ファイルは読み取り専用(表・
文書の複製にだけ書ける)、ネットワークは既定で遮断(利用者が net を明示した
...
貼る前に3つだけ見る — サンドボックスが守るのは**機械とネットワーク**で
あって、**結果の正しさは守らない**:
...
結果は undo 1手(気に入らなければ Ctrl+Z)、サンドボックス(暴れても機械に
触れない)。だから**まず回して、結果を見て、気に入らなければ戻す**が正しい
試し方。動きに納得したら、手続きは `~/.config/office/plugins/名前.py` に
置く(以後 `@名前` で回る)。**ブックに載せられるのは =PY の関数だけ**
(`@save 関数名` — 2026-08-08 に安全のため確定)。
