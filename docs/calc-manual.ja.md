...
- 図形はドラッグで移動、右下の角で大きさ変更、**枠の上の丸をドラッグで回転**
  (Shift で15度刻み)。選ぶと右に**図形の設定**の板(塗り・線の色と太さ・
...
| 文字列 | LEN, LEFT, RIGHT, MID, TRIM, UPPER, LOWER, PROPER, EXACT, CLEAN, CONCATENATE/CONCAT, **TEXT**, SUBSTITUTE, FIND, SEARCH, VALUE, NUMBERVALUE, TEXTJOIN, REPT, FIXED, YEN, CHAR/UNICHAR, CODE/UNICODE |
...
| 動的配列 | **FILTER, SORT, UNIQUE, SEQUENCE, TRANSPOSE** — 答えが隣のセルへあふれる(スピル)。先客がいれば `#SPILL!`。演算子との組み合わせ(`=SEQUENCE(3)+1`)も、集計への入れ子(`=SUM(FILTER(…))`)もできる |
...
- サンドボックス = bubblewrap(`apt install bubblewrap`)。**無い機械では、
...
### @コマンド — 関数はブックに、手続きは手元に

**ブックに載せられる(=帳票と一緒に旅ができる)のはセル関数(UDF)だけ**
(2026-08-08 に安全のため確定)。手続き(台帳への取り込みなどの台本)は
`~/.config/office/plugins/` に置いた .py だけが動く — **受け取ったファイルが
実行の起点になることはない**。
...
| 打つもの | 動き |
|---|---|
| `@save 関数名` | .py を選んでブックに載せる(**「関数」で始まる名前だけ** — =PY の定義。保存で xlsx に入る) |
| `@名前` | **plugins の .py を実行**(`~/.config/office/plugins/名前.py`。必ずネットワーク遮断のサンドボックスの中) |
| `@名前 net` | ネット許可つきのサンドボックスで実行(下記) |
| `@list`(または `@`) | ブックの関数と plugins の手続きの一覧 |
| `@del 名前` | 関数をブックから外す |
| `@export 名前` | 古いブックに載っている手続きを .py に取り出す(**実行はしない** — 中身を確かめてから自分で plugins へ) |
| `@計算` | =PY(…) セルの一括計算(下記) |

関数は xlsx の独自部品 `xl/joPython.xml` に入る。Excel で開いて保存し
直すとこの部品は消えることがある(**値は残る** — 劣化は安全側)。
昔の形式で手続きが載っているブックを開くと、その旨を報告に出す —
実行はできず、保存でブックから消える(`@export` で取り出せる)。

### サンドボックス

他所から来たかもしれないコードの実行環境は檻で閉じる(素の Linux は
bubblewrap、Flatpak 版は公式の入れ子檻):
**ネットワーク遮断・実ファイルは読み取り専用・ホームは不可視(空)・
書けるのは交換用の一時領域だけ**。さらに時間制限つき(手続き60秒・
関数30秒)— 無限ループでもアプリの手は塞がらない。

- ネットが要る仕事(Web フォームの受信箱から台帳へ取り込む等)は、
  **その場で `@名前 net` と打ったときだけ**開く。この許可はどこにも
  保存されない — 毎回が明示の同意
- 自分で打った/選んだコードも、サンドボックスがあればその中で回す
  (深層防御。こちらはネットワークあり)
...
- **マクロ**(プラグインタブ): .py を選ぶとサンドボックスの中の Python が回る
...
| マクロ(VBA) | **作らない方針**。Python in Calc(サンドボックスつき)が担う |
