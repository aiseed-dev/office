...
mod size_grip_tests {
    ...
    #[test]
    fn 一覧は押したボタンの真下に出る() {
        // 格子の面は窓の左端(x=0)から、リボンの下(y=120)に始まる、幅1200
        // 「ホーム」の書体のボタン(窓の左寄り)を押した
        assert_eq!(pop_x(300.0, 0.0, 1200.0), 288.0, "押した場所から離れて出た");
        // 面が窓の左端でないとき(将来の作業ウィンドウなど)も面の中で数える
        assert_eq!(pop_x(300.0, 200.0, 1000.0), 88.0, "面のずれを引いていない");
        // 右端のボタン: 一覧が窓の外へ出ないよう内に寄せる
        assert_eq!(pop_x(1190.0, 0.0, 1200.0), 960.0, "右端で一覧がはみ出す");
        // 左端のボタン: 面の外(負)へは出さない
        assert_eq!(pop_x(4.0, 0.0, 1200.0), 0.0, "左へはみ出した");
        // まだ一度も描いていない(幅0)ときは寄せずにそのまま
        assert_eq!(pop_x(300.0, 0.0, 0.0), 288.0, "幅が分かる前に寄せた");
    }
