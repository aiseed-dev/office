...
**HTML→文書は ONLYOFFICE に実装がある**(発注者 2026-08-04)— vendor/ の
sdkjs(HTML の貼り付け・変換)と x2t の html⇄docx が**挙動の教師**に使える:
どのタグをどの docx 構造に写すか・崩れた HTML をどこまで癒すかの対応表を
先人から読める(エンジンは継がない、はいつも通り)。ただし vendor/ は
この機械(saki)には無い — 元の機械で参照のこと。
