...
writer は 74/80(残る灰色6は下記 — 縦書きを含む)。
...
残る灰色6と、実装して分かった見立て:
- **ドロップキャップ** — 次の回転で可能。docx は pPr の w:framePr
  (w:dropCap="drop")で往復できる。組版は「頭の1字を大きな Cell として
  1行目に置き、その段落の行長を字の幅ぶん狭める」近似から
  (行単位の行長は break_para が1つの値しか取れないため)
...
- **ハイフネーション** — 品質の出る実装にはパターン辞書(Knuth-Liang)の
  同梱が要る。免許と大きさの検討から
