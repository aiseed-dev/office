...
writer の残りの灰色25のうち、**挿入系は方針確定**(2026-08-03 発注者確認):
- 図形・SmartArt・グラフ・テキストボックス等 — **Python で描いて画像として
  貼る**(SEKKEI「writer の挿入系」)。灰色のまま。貼る道(画像を挿入)と
  原文持ち越しは実装・検証済みなので、追加の実装は無い
