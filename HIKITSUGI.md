...
- **変更履歴** — 大物。編集モデル(平文+段落持ち越し)に挿入・削除の
  記録を足す設計から
- 図表番号・相互参照・図表目次 — フィールド(REF/SEQ)の設計が要る。
  しおりの錨は保存で残るようになったので土台はある
- ブックマーク(UI)— 錨の往復は済み。一覧と移動の UI を作れば釦にできる
- ドロップキャップ・コンテンツコントロール・配色・ハイフネーション — 未着手
